...
router.get('/', (req, res) => {
  ...
  const sql = includeInactive === 'true'
    ? 'SELECT id, name, active, delivery_day as deliveryDay, abbreviation, short_name as shortName, shipto_line1 as shipToLine1, shipto_line2 as shipToLine2, shipto_city as shipToCity, shipto_state as shipToState, shipto_zip as shipToZip, shipto_phone as shipToPhone, billto_line1 as billToLine1, billto_line2 as billToLine2, billto_city as billToCity, billto_state as billToState, billto_zip as billToZip, catalog_on as catalogOn, include_default as includeDefault, show_on_mobile as showOnMobile, terms, is_distributor as isDistributor, use_print_order as usePrintOrder FROM customers ORDER BY name ASC'
    : 'SELECT id, name, active, delivery_day as deliveryDay, abbreviation, short_name as shortName, shipto_line1 as shipToLine1, shipto_line2 as shipToLine2, shipto_city as shipToCity, shipto_state as shipToState, shipto_zip as shipToZip, shipto_phone as shipToPhone, billto_line1 as billToLine1, billto_line2 as billToLine2, billto_city as billToCity, billto_state as billToState, billto_zip as billToZip, catalog_on as catalogOn, include_default as includeDefault, show_on_mobile as showOnMobile, terms, is_distributor as isDistributor, use_print_order as usePrintOrder FROM customers WHERE active = 1 ORDER BY name ASC';
...
router.patch('/:id', (req, res) => {
  const { active, name, deliveryDay, abbreviation, shortName, showOnMobile, terms, isDistributor, usePrintOrder } = req.body;
  ...
  if (!hasActive && !hasMobile && !hasName && !dayProvided && !abbrProvided && !shortProvided && !shipToProvided && terms === undefined && isDistributor === undefined && usePrintOrder === undefined) {
  ...
  const fresh = db.prepare('SELECT id, name, active, delivery_day as deliveryDay, abbreviation, short_name as shortName, shipto_line1 as shipToLine1, shipto_line2 as shipToLine2, shipto_city as shipToCity, shipto_state as shipToState, shipto_zip as shipToZip, shipto_phone as shipToPhone, billto_line1 as billToLine1, billto_line2 as billToLine2, billto_city as billToCity, billto_state as billToState, billto_zip as billToZip, catalog_on as catalogOn, include_default as includeDefault, show_on_mobile as showOnMobile, terms, is_distributor as isDistributor, use_print_order as usePrintOrder FROM customers WHERE id = ?').get(req.params.id);
