...
router.get('/', (req, res) => {
  ...
  const sql = includeInactive === 'true'
    ? 'SELECT id, name, active, delivery_day as deliveryDay, abbreviation, short_name as shortName, shipto_line1 as shipToLine1, shipto_line2 as shipToLine2, shipto_city as shipToCity, shipto_state as shipToState, shipto_zip as shipToZip, shipto_phone as shipToPhone, billto_line1 as billToLine1, billto_line2 as billToLine2, billto_city as billToCity, billto_state as billToState, billto_zip as billToZip, catalog_on as catalogOn, include_default as includeDefault, show_on_mobile as showOnMobile, terms, is_distributor as isDistributor, use_print_order as usePrintOrder, hide_barcodes as hideBarcodes FROM customers ORDER BY name ASC'
    : 'SELECT id, name, active, delivery_day as deliveryDay, abbreviation, short_name as shortName, shipto_line1 as shipToLine1, shipto_line2 as shipToLine2, shipto_city as shipToCity, shipto_state as shipToState, shipto_zip as shipToZip, shipto_phone as shipToPhone, billto_line1 as billToLine1, billto_line2 as billToLine2, billto_city as billToCity, billto_state as billToState, billto_zip as billToZip, catalog_on as catalogOn, include_default as includeDefault, show_on_mobile as showOnMobile, terms, is_distributor as isDistributor, use_print_order as usePrintOrder, hide_barcodes as hideBarcodes FROM customers WHERE active = 1 ORDER BY name ASC';
...
router.patch('/:id', (req, res) => {
  const { active, name, deliveryDay, abbreviation, shortName, showOnMobile, terms, isDistributor, usePrintOrder, hideBarcodes } = req.body;
  ...
  if (!hasActive && !hasMobile && !hasName && !dayProvided && !abbrProvided && !shortProvided && !shipToProvided && terms === undefined && isDistributor === undefined && usePrintOrder === undefined && hideBarcodes === undefined) {
  ...
  if (typeof isDistributor === 'boolean') { updates.push('is_distributor = ?'); params.push(isDistributor ? 1 : 0); }
  if (typeof usePrintOrder === 'boolean') { updates.push('use_print_order = ?'); params.push(usePrintOrder ? 1 : 0); }
  if (typeof hideBarcodes === 'boolean') { updates.push('hide_barcodes = ?'); params.push(hideBarcodes ? 1 : 0); }
  ...
  const fresh = db.prepare('SELECT id, name, active, delivery_day as deliveryDay, abbreviation, short_name as shortName, shipto_line1 as shipToLine1, shipto_line2 as shipToLine2, shipto_city as shipToCity, shipto_state as shipToState, shipto_zip as shipToZip, shipto_phone as shipToPhone, billto_line1 as billToLine1, billto_line2 as billToLine2, billto_city as billToCity, billto_state as billToState, billto_zip as billToZip, catalog_on as catalogOn, include_default as includeDefault, show_on_mobile as showOnMobile, terms, is_distributor as isDistributor, use_print_order as usePrintOrder, hide_barcodes as hideBarcodes FROM customers WHERE id = ?').get(req.params.id);
