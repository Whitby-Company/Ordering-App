...
router.get('/', (req, res) => {
  ...
  const lineStmt = db.prepare(
    `SELECT ol.item_id as id, i.name, i.brand, ol.qty
     FROM order_lines ol
...
router.post('/', (req, res) => {
  ...
  const getItem = db.prepare('SELECT id, name, brand, stock FROM items WHERE id = ?');
  const resolvedLines = [];
  ...
  res.status(201).json({
    ...
    deliveryDate,
    submittedAt,
    lines: resolvedLines.map(({ item, qty }) => ({ id: item.id, name: item.name, brand: item.brand, qty })),
  });
});
